...
class MutationProjection {
  ...
  processMutations() {
    if (!this.treeChanges.anyParentsChanged &&
      !this.treeChanges.anyAttributesChanged) { return; }
  ...
  ensureHasOldPreviousSiblingIfNeeded(node: Node) {
    ...
    if (nodeChange && nodeChange.oldParentNode) { 
      parentNode = nodeChange.oldParentNode as Node & ParentNode; 
    }
  ...
  getCharacterDataChanged(): Node[] {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (!change.characterData ||
        target.textContent == change.characterDataOldValue) { continue }
  ...
  computeMatchabilityChange(selector: Selector, el: Element): Movement {
    ...
    if (!this.matchCache[selector.uid]) { this.matchCache[selector.uid] = new NodeMap<Movement>(); }
  ...
  processChildlistChanges() {
    ...
    for (let i = 0; i < this.mutations.length; i++) {
      ...
      if (this.treeChanges.reachabilityChange(mutation.target) !== Movement.STAYED_IN &&
        !this.calcOldPreviousSibling) { continue; }
      ...
      function recordOldPrevious(node: Node, previous: Node) {
        if (!node ||
          ...
          change.maybeMoved.has(node)) { return; }

        if (previous &&
          (change.added.has(previous) ||
            change.maybeMoved.has(previous))) { return; }
      ...
      if (query.attribute) {
        ...
        if (query.attributeList) {
          query.attributeList.forEach((attrName) => {
            if (!this.attributeChanged.hasOwnProperty(attrName)) { this.attributeChanged[attrName] = []; }
          });
        }
      }
    }
...
class Qualifier {
  ...
  public toString(this: Qualifier): string {
...
function elementFilterAttributes(selectors: Selector[]): string[] {
  const attributes: StringMap<boolean> = {};
...
export class MutationSummary {
  ...
  private changesToReport(summaries: Summary[]): boolean {
    return summaries.some((summary) => {
      ...
      if (summaryProps.some(function (prop) { return summary[prop] && summary[prop].length; })) { return true; }
